...
export default Hgraph<graphs.NetworkProps, graphs.CustomNode>(
    ({ width, height,
      hideTooltip,
      showTooltip,
      tooltipOpen,
      ...
      const handleMouseMove = useCallback(
        (event: React.MouseEvent | React.TouchEvent) => {
          ...
          if (closest) {
            ...
            temp = graphs.hnodes.find(({x,y}) => x==closest.data.x && y==closest.data.y )
            if(temp){
              temp!.color =  "#FFFFFF"
      ...
      return width < 10 ? null : (
        ...
        <svg width={width} height={height} ref={svgRef}
          ...
          {Math.round((graphs.dist_between_routes(graphs.heur_method, graphs.heur_matrix) + Number.EPSILON) * 100) / 100}</text>
        </svg>
